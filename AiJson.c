         ...
         if(**str)
         {
            ...
            if(**str == CHAR_UNICODE_CTRL)
            {
               ...
               for(j = 0; j < NUM_UNICODE_CTRL_DIGITS; ++j)
               {
                  ...
                  if(!hex[j] || !isxdigit(hex[j]))
                     return AI_JSON_E_INVALID_HEX_DIGIT;
               }
               hex[j] = 0;

               errno = 0;
               codePoint = strtol(hex, &end, 16);
               if(errno != 0 || hex == end)
...
{
   if(!curr && (isdigit(next) || next == CHAR_MINUS)) 
      return AI_JSON_E_OK;    
   else if(curr == CHAR_DECIMAL && isdigit(next))
      return AI_JSON_E_OK;  
   else if(curr == CHAR_EXPONENT && (isdigit(next) || next == CHAR_PLUS || next == CHAR_MINUS))
      return AI_JSON_E_OK;
   else if((curr == CHAR_MINUS || curr == CHAR_PLUS) && (isdigit(next)))
      return AI_JSON_E_OK;
   ...
   {
      if(isdigit(curr) && (isdigit(next) || (!exponent && next == CHAR_EXPONENT || (next == CHAR_DECIMAL && !decimal))))
         return AI_JSON_E_OK;
   }
...
AiJsonError ParseNumber(AiJson *json, char **str, AiJsonValue *value)
{
   ...
   for(i = 1;; ++i)
   {
      ...
      if(err != AI_JSON_E_OK)
         return err;
      curr = next;
      next = tolower(IncreaseStr(json, str, 1));
   ...
   if(!num)
      return AI_JSON_E_ALLOC;
   num[i] = 0;
   memcpy(num, numStart, i);

   errno = 0;
   if(decimal)
   {
      value->type     = AI_JSON_DOUBLE;
      value->data.dbl = strtod(num, &numStart);
   }
   else
   {
      value->type         = AI_JSON_SLONG;
      value->data.sLong   = strtol(num, &numStart, 10);
      if(errno != 0 || num == numStart)
      {  
         errno = 0;
         value->type       = AI_JSON_ULONG;
         value->data.uLong = strtoul(num, &numStart, 10);
         printf("%d", errno); 
      }
   }
   if(errno != 0 || num == numStart)
...
AiJsonError ParseValue(AiJson *json, char **str, AiJsonValue *value)
{
   ...
   switch(**str)
   {
      ...
      case CHAR_FIRST_FALSE:
      {
         ...
         if(!strncmp(*str, STR_TRUE, sizeof(STR_TRUE) - 1))
         {
            value->data.boolean = 1;
            IncreaseStr(json, str, sizeof(STR_TRUE) - 1);
         }
         else if(!strncmp(*str, STR_FALSE, sizeof(STR_FALSE) - 1))
         {
      ...
      case CHAR_FIRST_NULL:
      {
         ...
         if(strncmp(*str, STR_NULL, sizeof(STR_NULL) - 1))
            return AI_JSON_E_INVALID_VALUE;
...
{  
   ...
   json = (AiJson *) malloc(sizeof(*json));
   if(!json)
      goto err;
...
AiJson *AiJsonParse(char *str)
{
   ...
   if(!strncmp(str, utf8bom, sizeof(utf8bom) - 1))
...
{   
   ...
   while(curr)
   {
      ...
      switch(value->type)
      {
         ...
         case AI_JSON_DOUBLE:
         {
            sprintf(buff, "%f", value->data.dbl);
            AiStrAppenderWork(appender, buff);
            break;
         }
         case AI_JSON_ULONG:
         {
            sprintf(buff, "%lu", value->data.uLong);
            AiStrAppenderWork(appender, buff);
            break;
         }
         case AI_JSON_SLONG:
         {
            sprintf(buff, "%ld", value->data.sLong);
            AiStrAppenderWork(appender, buff);
            break;
         }
         case AI_JSON_BOOL:
